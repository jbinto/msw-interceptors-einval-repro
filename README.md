...
Everything runs in Docker. There are two service variants, both driving real `https.request` passthrough through the **real published** interceptor:

| Service | Stack | Used for |
|---|---|---|
| `repro-work-tls` | nock@14.0.10 → interceptors@**0.39.8** | the stack we run at work — the main `EINVAL` / `ECANCELED` rig |
| `repro-latest-tls` | nock@14.0.15 → interceptors@**0.41.9** | the latest published stack — used for the #757 memory control |

**Prerequisite:** Docker Desktop set to **dual IPv4/IPv6** (*Settings → Resources → Network → Default networking mode → Dual IPv4/IPv6*). IPv4-only and Simulator 1 can't reproduce `EINVAL`. (`tc netem` needs `privileged`, already set in compose.)

**Build once** (compose won't rebuild on file changes), then each run is `docker compose run --rm [-e FLAG…] <service> [node <script>.mjs]`:

```bash
docker compose build repro-work-tls repro-latest-tls

# default harness (run-tls.mjs) — drive passthrough and count failures
docker compose run --rm -e CONCURRENCY=300 -e REQUESTS=8000 repro-work-tls

# inject the fix into the installed package first, then run
docker compose run --rm -e MSW_FIX=1 -e FIX_KIND=readnoop-noalias \
  -e CONCURRENCY=300 -e REQUESTS=8000 repro-work-tls

# run a specific script instead of the default harness
docker compose run --rm -e NO_NETEM=1 repro-work-tls node mechanism.mjs
```

Flags fall into two groups, all passed with `-e`:

- **harness** ([`latest/run-tls.mjs`](latest/run-tls.mjs)): `SERVER_MODE` (`drain`/`throttle`/`stall`/`tlsstall`), `BODY_KB`, `CONCURRENCY`, `REQUESTS`, `RUNS`, `MAX_SOCKETS`, `TARGET_HOST`, `REQ_TIMEOUT_MS`, `DESTROY_ON_TIMEOUT`, `SERVER_BIND`
- **entrypoint** ([`latest/docker-entrypoint.sh`](latest/docker-entrypoint.sh), runs before the harness): `NO_NETEM` (skip the loopback delay), `TINY_BUF`, `POISON_V6`, `MSW_FIX` + `FIX_KIND` (inject the fix), `INSTRUMENT`

The sections below are worked examples of exactly these commands.
...
Everything lives in `latest/` — the published-package testbed. Each script runs via `docker compose run --rm … <service> node <script>.mjs`:

| File | What |
|---|---|
| `latest/run-tls.mjs` | default HTTPS passthrough harness (Simulators 1 & 2) |
| `latest/mechanism.mjs` | deterministic write-face primitive (paths A–D) |
| `latest/apply-fix.mjs` | injects a `FIX_KIND` into the *installed* package at container start |
| `latest/regression706.mjs` | #706 listener-leak regression guard |
| `latest/mem.mjs` | #757 memory-leak profiler |
| `latest/instrument.mjs` | logs socket write-state at destroy (debugging, via `INSTRUMENT`) |
| `latest/docker-entrypoint.sh` | sets up `tc netem` and applies the entrypoint flags before the harness |

> **The original Dec 2025 reproduction** — minimal and HTTP-only, built from this fork's own `src/` (the "https not needed to repro" era, before any of the `ECANCELED`/TLS work) — lived at [`a42a692`](https://github.com/jbinto/msw-interceptors-einval-repro/tree/a42a692). It's been removed from the working tree; that commit is the snapshot of what it looked like.
